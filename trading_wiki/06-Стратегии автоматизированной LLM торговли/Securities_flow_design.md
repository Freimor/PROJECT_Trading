...
> Отдельный n8n workflow для **ценных бумаг MOEX**: учитывает **торговые сессии**, расчёты **T+1**, бесплатные данные **MOEX ISS** и ордера через **T-Invest API** (T-Bank). Два подрежима: **index_dca** (ежемесячная покупка БПИФ) и **swing_signals** (сигналы по ликвидным акциям).

---

## Для новичка

Securities-flow отличается от crypto-flow:

| Аспект | Crypto | Securities (MOEX) |
|--------|--------|-------------------|
| Часы работы | 24/7 | ~10:00–18:50 MSK (основная сессия) |
| Settlement | Мгновенно (spot) | **T+1** — акции «приходят» на следующий день |
| Данные | Binance API | **MOEX ISS** (бесплатно) |
| Ордера | Binance API | **T-Invest API** (брокер) |
| Лоты | Дробные (0.001 BTC) | **Кратность лоту** (SBER = 10 акций) |
| Налоги | Отдельный учёт | НДФЛ, ИИС — см. [[Russia_tax_basics]] |

**MOEX ISS** даёт котировки без регистрации. **T-Invest API** — для реальных заявок через брокерский счёт.
