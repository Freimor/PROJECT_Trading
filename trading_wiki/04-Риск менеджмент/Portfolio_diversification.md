...
> **Диверсификация (diversification)** — распределение инвестиций между **разными активами, секторами, рынками и стратегиями**, чтобы снизить влияние неудачи **одной** позиции на весь портфель. SEC формулирует принцип: «Don't put all your eggs in one basket» — не кладите все яйца в одну корзину.

---

## Для новичка

Вы купили акции **одной** компании на весь капитал. Компания объявляет банкротство → потеря **всего** счёта.

Если же капитал распределён между **несколькими** компаниями, облигациями и cash — падение одной бумаги **ограничивает**, но не уничтожает портфель.

Investor.gov объясняет: факторы, ухудшающие один класс активов, могут **улучшить** доходность другого. Диверсификация **внутри** класса (несколько акций, секторов) дополняет диверсификацию **между** классами (акции + облигации + cash).

**Важно для трейдера:** диверсификация ≠ «купить 10 altcoin». Если все altcoin **движутся вместе с BTC**, реального снижения риска может не быть — нужна **корреляция**, а не только количество тикеров.
...
| # | Факт | Источник |
|---|------|----------|
| 1 | **Diversification** — практика распределения денег между разными инвестициями для **снижения риска**; «не кладите все яйца в одну корзину». | [Investor.gov: Asset Allocation](https://www.investor.gov/introduction-investing/getting-started/asset-allocation) |
| 2 | Диверсификация **внутри класса**: несколько акций/облигаций, разные **отрасли** (consumer, health care, technology) — если один сектор падает, другие могут компенсировать. | [Investor.gov: Asset Allocation](https://www.investor.gov/introduction-investing/getting-started/asset-allocation) |
| 3 | **Mutual funds и ETF** упрощают диверсификацию (много бумаг в одном продукте), но **узко сфокусированный** фонд (один сектор) **не гарантирует** diversification — проверяйте top holdings. | [Investor.gov: Asset Allocation](https://www.investor.gov/introduction-investing/getting-started/asset-allocation) |
| 4 | **Inadequate diversification** — портфель **слишком сконцентрирован** в одном типе инвестиций; **увеличивает** risk exposure. | [SEC: Behavioral Patterns](https://www.investor.gov/introduction-investing/general-resources/news-alerts/alerts-bulletins/investor-bulletins-72) |
| 5 | **Naïve diversification** — равномерное распределение между N опциями **без учёта** уровня риска каждой; может **увеличить** risk exposure в зависимости от активов. | [SEC: Behavioral Patterns](https://www.investor.gov/introduction-investing/general-resources/news-alerts/alerts-bulletins/investor-bulletins-72) |
| 6 | **Familiarity bias** — предпочтение «знакомых» инвестиций (своя страна, glamour stocks); может привести к **недостаточной** диверсификации. | [SEC: Behavioral Patterns](https://www.investor.gov/introduction-investing/general-resources/news-alerts/alerts-bulletins/investor-bulletins-72) |
| 7 | **Rebalancing** — возврат портфеля к целевым весам; при росте акций с 60% до 80% нужно **продать часть** и перераспределить; «buy low, sell high» через дисциплину. | [Investor.gov: Asset Allocation](https://www.investor.gov/introduction-investing/getting-started/asset-allocation) |
| 8 | FINRA: при высокой **volatility** легко **импульсивно** продать всё; **diversification** — один из способов управлять волатильностью и тревогой. | [FINRA: Volatility](https://www.finra.org/investors/investing/investing-basics/volatility) |
| 9 | SEC (FOMO): не класть **все деньги в 1–2 акции**; распределять по **секторам**; смесь stocks/bonds/cash снижает impact volatility. | [Investor.gov: Say NO GO to FOMO](https://www.investor.gov/additional-resources/spotlight/formerdirectorlorischock-directors-take/say-no-go-fomo) |
